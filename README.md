...
│   │   ├── ports.py · enrichment.py · tls.py · intel.py
│   │   ├── hygiene.py  — findings + exposure map + posture grade
│   │   ├── tags.py     — asset role/owner annotations
│   │   ├── report.py   — normalized schema + HTML/JSON/CSV/MD writers
...
│   │   └── cli.py      — monitor scan|history|diff|ack|certs|assets|score|alerts
...
├── examples/           — sample alerts.json + asset_tags.json
...
python -m omnirecon scan --export csv,md
```
...
ntfy via one URL), an OS desktop toast, and an always-on `reports/alerts.log`.
...
| **Extra exports** (CSV · Markdown, on top of HTML+JSON) | `--export csv,md` |
| **Monitor alerting** (webhook · desktop · log on qualifying deltas) | `reports/alerts.json` |
...
> `paramiko`, `smb-enum` needs `smbprotocol`. Missing ones are skipped, not fatal.
